...
interface EleVateAchievementsProps {
  gameId: string;
  competitorId: string;
}

const EleVateAchievements: React.FC<EleVateAchievementsProps> = ({ gameId, competitorId }) => {
  ...
  useEffect(() => {
    const fetchAchievements = async () => {
      try {
        const response = await fetchWithAuth(`/api/achievements?game=${gameId}&competitor=${competitorId}`);
        if (!response.ok) {
...
export default EleVateAchievements;
